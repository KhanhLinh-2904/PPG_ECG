...
def train_epoch(
    ...
    contrast_loss_fn, mse_loss_fn, pearson_loss_fn, distribution_loss_fn,
    device, scaler, current_epoch
):
    model.train()
    metrics = {k: 0.0 for k in ['total', 'contrast', 'mse', 'pearson', 'distribution']}
    ...
    for ecg, ppg, _ in progress_bar:
        ...
        with autocast():
            ...
            # d_loss = distribution_loss_fn(embed_ecg, embed_ppg)
            
            total_loss = (WEIGHT_CONTRASTIVE * c_loss + 
                          WEIGHT_MSE * m_loss + 
                          WEIGHT_PEARSON * p_loss )
                        #   WEIGHT_DISTRIBUTION * d_loss)
            
            # m_loss_val, p_loss_val, d_loss_val = m_loss.item(), p_loss.item(), d_loss.item()
        ...
        # metrics['distribution'] += d_loss_val

        progress_bar.set_postfix({
            'Total': f"{total_loss.item():.4f}", 
            'Cont': f"{c_loss.item():.3f}",
            # 'Dist': f"{d_loss_val:.3f}",
            'MSE': f"{m_loss_val:.4f}"
        })
...
def evaluate(model, dataloader, contrast_loss_fn, mse_loss_fn, pearson_loss_fn, distribution_loss_fn, device):
    model.eval()
    t_mse, t_pearson, t_contrast, t_dist = 0.0, 0.0, 0.0, 0.0
    with torch.no_grad():
        for ecg, ppg, _ in dataloader:
            ...
            # t_dist += distribution_loss_fn(embed_ecg, embed_ppg).item()
            
    n = len(dataloader)
    avg_corr = 1.0 - (t_pearson / n)
    return t_mse/n, avg_corr, t_contrast/n, t_dist/n
...
if __name__ == "__main__":
    ...
    distribution_fn = DistributionAlignmentLoss(eps=1e-8).to(device)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        avg_train, last_emb_ecg, last_emb_ppg = train_epoch(
            model, train_loader, optimizer, contrast_fn, mse_fn, pearson_fn, distribution_fn, device, scaler, epoch
        )
        
        test_mse, test_corr, test_cont, test_dist = evaluate(
            model, test_loader, contrast_fn, mse_fn, pearson_fn, distribution_fn, device
        )
        
        history['Total Loss'].append(avg_train['total'])
        history['Contrastive Loss'].append(avg_train['contrast'])
        history['Distribution Loss'].append(avg_train['distribution'])
        ...
        print(f"[Train] Total: {avg_train['total']:.4f} | Cont: {avg_train['contrast']:.4f} | Dist: {avg_train['distribution']:.4f}")
        print(f"[Test]  MSE: {test_mse:.6f} | Corr: {test_corr:.4f} | Cont: {test_cont:.4f} | Dist: {test_dist:.4f}")
