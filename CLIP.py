...
class DecoderBlock_UNet(nn.Module):
    """Giữ nguyên: Một khối Upsampling + Conv1D."""
    ...
    def forward(self, x, skip=None):
        ...
        if skip is not None:
            ...
            x = torch.cat([x, skip], dim=1) # Nối theo chiều channel
...
class ECGDecoder_UNet(nn.Module):
    def __init__(self, bottleneck_channels=2048):
        ...
        # Block 1: Input 512 + Skip f3 (1024) -> Output 256. Upsample 75 -> 150
        self.block1 = DecoderBlock_UNet(in_channels=512, out_channels=256, skip_channels=1024)
        
        # Block 2: Input 256 + Skip f2 (512) -> Output 128. Upsample 150 -> 300
        self.block2 = DecoderBlock_UNet(in_channels=256, out_channels=128, skip_channels=512)
        
        # Block 3: Input 128 + Skip f1 (256) -> Output 64. Upsample 300 -> 600
        self.block3 = DecoderBlock_UNet(in_channels=128, out_channels=64, skip_channels=256)
        
        # Block 4: Input 64 + Skip None -> Output 32. Upsample 600 -> 1200
        # (ResNet layer đầu tiên downsample mạnh nên không còn feature map ở 1200 để skip, trừ khi lấy sau conv1)
        self.block4 = DecoderBlock_UNet(in_channels=64, out_channels=32, skip_channels=0)
        
        # Block 5: Input 32 + Skip None -> Output 16. Upsample 1200 -> 2400
    ...
    def forward(self, z, features_list):
        # z: (B, 2048, 75) - Bottleneck
        # features_list: [f1, f2, f3] tương ứng size [600, 300, 150]
        
        f1, f2, f3 = features_list # Unpack skip connections
        
        x = self.adapter(z) # -> (B, 512, 75)
        
        # Bắt đầu giải mã + Nối tắt
        x = self.block1(x, skip=f3) # 75->150. Cat với f3
        x = self.block2(x, skip=f2) # 150->300. Cat với f2
        x = self.block3(x, skip=f1) # 300->600. Cat với f1
        
        # Các lớp cuối upsample đơn thuần để về 2400
        x = self.block4(x) # 600->1200
        x = self.block5(x) # 1200->2400
...
class PPGtoECGConverter(nn.Module):
    def __init__(self, embed_dim=OUTPUT_EMBED_DIM, input_length=INPUT_LENGTH):
        ...
        self.ecg_decoder = ECGDecoder_UNet(
            bottleneck_channels=2048
        )

    def forward(self, z_ppg):
