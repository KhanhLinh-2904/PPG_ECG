...
class SignalProcessor:
    ...
    def normalize_signal(self, signal_data: np.ndarray) -> np.ndarray:
        # Bước 1: Z-Normalization (Đưa về Mean = 0, Std = 1)
        ...
        # Bước 2: Min-Max Scaling về dải [-1, 1]
        min_val = np.min(z_norm_signal)
        max_val = np.max(z_norm_signal)
        
        # Tránh chia cho 0 nếu toàn bộ tín hiệu là một đường thẳng
        if max_val - min_val < 1e-8:
            return np.zeros_like(signal_data)
            
        # Công thức scale từ dải [min, max] sang [-1, 1]:
