...
def get_coral_stats(features):
    """Calculate Mean and Covariance of the Test set (Used for alignment)"""
    mu = np.mean(features, axis=0)
    # Add a small value to the diagonal for numerical stability (invertibility)
    cov = np.cov(features, rowvar=False) + np.eye(features.shape[1]) * 1e-5
    return mu, cov

def apply_coral(source_mu, source_cov, target_features):
    """
    Aligns the Test (Target) data distribution to match the Train (Source) distribution.
    - source_mu, source_cov: Extracted from the pickle file (MIT-BIH stats)
...
# Path to the file containing CORAL stats and the trained classifier
coral_model_path = 'AF_Detection/checkpoints/coral_mlp_checkpoint.pkl'
...
if __name__ == "__main__":
    ...
    # ==============================================================================
    # 1. LOAD CLASSIFIER & CORAL STATS FROM PHASE 2
    # ==============================================================================
    print("📚 Loading Classifier and CORAL parameters...")
    try:
        ...
        print(f"❌ Error: Pickle file is missing key {e}. The file might be corrupted or outdated.")
        exit()
    ...
    try:
        ...
        print(f"❌ Error: Could not find backbone weights at {pretrained_path}")
        exit()

    backbone.eval() 

    # ==============================================================================
    # 3. RUN PREDICTION ON EACH DATASET
    # ==============================================================================
    print("\n" + "="*60)
    print("🔍 PERFORMING EVALUATION (CORAL ALIGNMENT + CLASSIFICATION)...")
    print("="*60)
    ...
    for test_name, file_path in test_files.items():
        ...
        with torch.no_grad():
            for inputs, labels in test_loader:
                ...
                # Global Average Pooling (mean across sequence length)
                features = outputs['local_reps'].mean(dim=1).cpu().numpy()
                all_test_features.append(features)
                all_true_labels.extend(labels.numpy())

        # Concatenate into feature matrix (N_samples, 256)
        all_test_features = np.concatenate(all_test_features, axis=0)
        all_true_labels = np.array(all_true_labels)
      
        # ==============================================================================
        # 3.2: APPLY CORAL ALIGNMENT
        # ==============================================================================
        print(f"    🔄 Aligning data space using CORAL Alignment...")
        # Warps the test set feature cloud to match the training set distribution
        ...
        if cm.shape == (2, 2):
            ...
            # Handle cases where only one class is present in the test set
