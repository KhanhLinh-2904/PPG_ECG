...
# [THAY ĐỔI 1]: Import LatentTransformerFlow thay vì CNN
...
# [THAY ĐỔI 2]: Trỏ tới thư mục lưu trọng số của Transformer
...
# [THAY ĐỔI 3]: Thông số kiến trúc của Transformer
...
def load_models():
    ...
    # [THAY ĐỔI 4]: Khởi tạo bằng mô hình Transformer
...
def align_signals(true_s: np.ndarray, pred_s: np.ndarray) -> np.ndarray:
    ...
    if lag > 0:
        ...
        # Padding phần hụt ở bên trái bằng giá trị ĐẦU TIÊN của tín hiệu
        edge_value = pred_s[0]
        ...
        # Padding phần hụt ở bên phải bằng giá trị CUỐI CÙNG của tín hiệu
        edge_value = pred_s[-1]
        aligned_pred[-lag:] = edge_value
    ...
    with torch.no_grad():
        for batch_data in dataloader:
            ...
            for i in range(ppg.shape[0]):
                # Chuyển đổi sang Numpy 1D
                ppg_plot = ppg[i].squeeze().cpu().numpy()
                ecg_gt_plot = ecg[i].squeeze().cpu().numpy()
                # Lấy trực tiếp tín hiệu dự đoán không qua alignment
                ...
                # Vẽ trực tiếp tín hiệu dự đoán gốc
                plt.plot(t, ecg_pred_plot, color='red', label='Predicted ECG', linestyle='--', alpha=0.8)
