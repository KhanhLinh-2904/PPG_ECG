...
PHASE2_MODEL_PATH = '/home/linhhima/PPG_ECG/saved_models_alignment_new/best_ppg_alignment.pth'
...
def load_model():
    ...
    if not os.path.exists(PHASE1_MODEL_PATH):
        raise FileNotFoundError(f"Lỗi: Cần trọng số Phase 1: {PHASE1_MODEL_PATH}")
...
def run_tsne_layer_evolution():
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader, desc="Forward Pass"):
            ...
            if not data_warning_triggered and torch.allclose(ecg, ppg, atol=1e-5):
                warnings.warn("CẢNH BÁO: Tín hiệu PPG và ECG hoàn toàn giống nhau!")
    ...
    # ---------------------------------------------------------
    # BƯỚC 6 & 7 (CŨ): MẬT ĐỘ T-SNE KDE (Tùy chọn, đã ẩn để tập trung hình mới)
    # ---------------------------------------------------------
    # Các bước này tính mật độ trên tọa độ méo mó của t-SNE.
    # Đã được thay thế bằng hình Gaussian chuẩn ở bước 8 & 9 bên dưới.

    # ---------------------------------------------------------
    # BƯỚC 8: VẼ ĐƯỜNG CONG GAUSSIAN 1D (Giống Hình 1)
    # ---------------------------------------------------------
    print("\n8. Running Theoretical Gaussian Curve for Final Latent Space...")
    # Tính Mean và Std thực tế trên toàn bộ ma trận Latent (làm phẳng thành mảng 1 chiều)
    ...
    # Tạo trục X để vẽ đồ thị bao trùm cả 2 phân phối
    ...
    # ---------------------------------------------------------
    # BƯỚC 9: VẼ MẶT CONG GAUSSIAN 3D (Giống Hình 2)
    # ---------------------------------------------------------
    ...
    # Độ lệch thực tế giữa 2 phân phối
    shift = mu_p - mu_e
    ...
    # Khởi tạo phân phối chuẩn đa biến (Multivariate Normal)
    # ECG làm gốc (0,0), PPG bị dịch đi một đoạn `shift`
    rv_e = stats.multivariate_normal([0, 0], [[std_e**2, 0], [0, std_e**2]])
    rv_p = stats.multivariate_normal([shift, shift], [[std_p**2, 0], [0, std_p**2]])

    # Vẽ 2 mặt cong lồng vào nhau
    ...
    # Ẩn các mốc số trục Z để giống hình minh họa
    ...
    # Xoay góc nhìn (Elevation = 30, Azimuth = -45)
