...
def calculate_frechet_distance(y_true, y_pred):
    ...
    # Khởi tạo ma trận khoảng cách tích lũy với giá trị -1.0 làm cờ đánh dấu
    ca = np.full((n, m), -1.0)
    
    # Cấu hình ô cơ sở biên (0, 0)
    ...
    # Khớp quy hoạch động xuyên suốt ma trận
    for i in range(1, n):
        for j in range(1, m):
            cost = abs(y_t[i] - y_p[j])
            # Lấy giá trị nhỏ nhất từ 3 ô lân cận trước đó
