...
class SinusoidalPosEmb(nn.Module):
    """ Nhúng vị trí thời gian t thành vector đặc trưng """
...
class FiLMBlock1D(nn.Module):
    """ Khối ResNet 1D kết hợp điều kiện thời gian t qua cơ chế FiLM (Adaptive Layer Norm) """
    ...
    def forward(self, x, t_emb):
        # Tính toán scale và shift từ thời gian t
        scale, shift = self.mlp(t_emb).unsqueeze(-1).chunk(2, dim=1)
        
        res = x
        x = self.norm1(x)
        x = x * (scale + 1) + shift  # Cấy điều kiện thời gian t vào đặc trưng
...
class LatentRectifiedFlow(nn.Module):
    ...
    def __init__(self, latent_channels=16, cond_channels=16, hidden_dim=128, num_blocks=6):
        ...
        # Gộp x_t và z_ppg (điều kiện c) ở đầu vào
    ...
    def forward(self, xt, t, z_ppg):
        """
        xt: [Batch, 16, 75] - Nhiễu lai với Target
        t: [Batch] - Bước thời gian từ 0 đến 1
        z_ppg: [Batch, 16, 75] - Latent PPG đã căn chỉnh (Conditioning)
        """
        t_emb = self.time_mlp(t)
        
        # Nối x_t và z_ppg dọc theo chiều channels
        ...
        out_v = self.conv_out(h) # Dự đoán vector vận tốc v
        return out_v
