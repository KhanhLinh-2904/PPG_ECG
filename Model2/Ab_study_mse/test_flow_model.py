...
PHASE1_PPG_PATH = '/home/linhhima/PPG_ECG/Model2/Ab_study_mse/saved_models_alignment_segment/best_ppg_alignment.pth'
PHASE2_FLOW_PATH = '/home/linhhima/PPG_ECG/Model2/Ab_study_mse/saved_models_flow_segment/best_rectified_flow.pth'
...
#     trong DataLoader mà không phân biệt tên record.
...
#     print(f"[*] Visualizing segments. Max plots configured: {max_plots}")
#     plot_count = 0
...
#                 # Kiểm tra điều kiện dừng nếu đã đạt giới hạn max_plots đặt ra
#                 if max_plots is not None and plot_count >= max_plots:
#                     print(f"\n[*] Đã hiển thị đủ giới hạn {max_plots} segments. Dừng xử lý rộng rãi để bảo vệ bộ nhớ!")
#                     return
...
def run_loss():
    ...
    # Hàm phụ trợ tạo từ điển lưu trữ metrics rỗng (Đã thêm frechet)
    ...
    with torch.no_grad():
        for batch_data in tqdm(test_loader, desc="Evaluating Metrics"):
            ...
            for b in range(ecg_true_np.shape[0]):
                ...
                # 1. Tính toán các chỉ số cho tín hiệu trước khi align
                ...
                frechet_b = calculate_frechet_distance(true_s, pred_s) # Chèn Frechet trước align
                
                # Căn chỉnh pha tín hiệu
                aligned_pred_s = align_signals(true_s, pred_s)
                
                # Tính toán các chỉ số sau khi align
                ...
                frechet_a = calculate_frechet_distance(true_s, aligned_pred_s) # Chèn Frechet sau align
    ...
    def print_report(title, m_dict):
        ...
        print(f"Fréchet      | {m_dict['before']['frechet']/count:<15.4f} | {m_dict['after']['frechet']/count:<15.4f}") # In Frechet
        print(f"{'='*55}")
...
def run_peak_count_evaluation():
    ...
    print("[*] Đang đếm và tính toán sai số đỉnh R (MAE & Error Percentage)...")
    ...
    with torch.no_grad():
        for batch_data in tqdm(test_loader, desc="Evaluating R-Peaks Errors"):
            ...
            for b in range(ecg_true_np.shape[0]):
                ...
                # Lấy số lượng đỉnh từ thuật toán (bỏ qua giá trị ratio cũ)
                num_true, num_pred, _ = calculate_peak_count_ratio(true_s, pred_s, sampling_rate=125)
                
                # =========================================================
                # TÍNH TOÁN SAI SỐ TỪNG SEGMENT THEO ĐÚNG CÔNG THỨC MỚI
                # =========================================================
    ...
    def print_peak_report(title, tracker):
        ...
        # Tính trung bình trên tổng số segments
        mae_peaks = tracker['sum_abs_err'] / count
        mape_peaks = tracker['sum_err_pct'] / count
        
        print(f"\n{'='*55}")
        print(f"BÁO CÁO SAI SỐ ĐỈNH R: {title.upper()} ({count} segments)")
        print(f"{'='*55}")
...
if __name__ == "__main__":
    ...
    # run_visualization()
    # run_loss()
    save_ecg_reconstruction("/home/linhhima/PPG_ECG/AF_Detection/total_mimic_af_flow_segment_mse.npz")
