...
class SignalTransformer(nn.Module):
    """
    ViT Variant for 1D Signal Data
    Input: 1D PPG signal (batch, channels=1, length)
    Output: Feature vector (batch, output_dim)
    """
    def __init__ (self, input_length: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        super(SignalTransformer, self).__init__()

        self.input_length = input_length
        self.output_dim = output_dim
        self.conv1 = nn.Conv1d(in_channels=1, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False)
