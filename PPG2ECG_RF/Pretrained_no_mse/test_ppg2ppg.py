...
# Paths for test dataset and PPG AE model checkpoint
TEST_DATA_PATH = '/home/linhhima/Diffusion_datasets/combined_segment_split_test.npz'
MODEL_PATH = '/home/linhhima/PPG_ECG/PPG2ECG_RF/Pretrained_no_mse/saved_models_ppg_ae_segment/best_ppg_autoencoder.pth'
...
def load_model():
    ...
    # Initialize configuration for PPG AE matching model design
    cfg = PPGAEConfig(
    ...
    # Load weights (supports checkpoint saved as a dict or raw state_dict)
...
def run_visualization():
    ...
    print("\n[*] Running inference for PPG signal visualization...")
    with torch.no_grad():
        for batch_data in test_loader:
            # batch_data[0] is ECG, batch_data[1] is PPG.
            # For PPG-to-PPG reconstruction, retrieve PPG at index 1.
            ppg = batch_data[1]
            record_names = batch_data[2]  # Assuming index 2 contains record names

            # Ensure 1D CNN input format [B, 1, 2400]
            ppg_input = ppg.float().unsqueeze(1).to(DEVICE) if ppg.dim() == 2 else ppg.float().to(DEVICE)

            # PPGAutoencoder model outputs a dictionary
            ...
            for idx in range(ppg_true_np.shape[0]):
                ...
                # Show only 5 demo samples and terminate
                if len(seen_records) >= 5:
                    print("[*] Displayed 5 demo samples for PPG visualization.")
                    return

def align_signals(true_s: np.ndarray, pred_s: np.ndarray) -> np.ndarray:
    """ Uses Cross-Correlation to find phase shift and align predicted signal to ground truth. """
...
def run_loss():
    ...
    print(f"\n{'='*52}")
    print(f"PPG AUTOENCODER EVALUATION RESULTS ({total_samples} samples)")
    print(f"{'='*52}")
