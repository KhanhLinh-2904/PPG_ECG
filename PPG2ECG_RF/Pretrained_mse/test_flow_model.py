...
# Checkpoint paths (Ensure paths are correct)
PHASE1_ECG_PATH = '/home/linhhima/PPG_ECG/PPG2ECG_RF/Proposed/saved_models_ecg_vae_segment/best_ecg_autoencoder.pth'
PHASE1_PPG_PATH = '/home/linhhima/PPG_ECG/PPG2ECG_RF/Pretrained_mse/saved_models_alignment_segment/best_ppg_alignment.pth'
PHASE2_FLOW_PATH = '/home/linhhima/PPG_ECG/PPG2ECG_RF/Pretrained_mse/saved_models_flow_segment/best_rectified_flow.pth'
...
def get_integrated_energy(signal, fs=125):
    """Auxiliary function computing integrated energy to render the threshold line."""
    nyq = 0.5 * fs
...
def pan_tompkins_qrs(ecg_signal: np.ndarray, fs: int = 125, external_threshold: float = None, return_threshold: bool = False):
    """
    Modified Pan-Tompkins algorithm to accept or return detection threshold.
    """
...
def calculate_peak_count_ratio(true_s: np.ndarray, pred_s: np.ndarray, sampling_rate=125):
    """
    Counts R-peaks using the Pan-Tompkins algorithm and computes peak ratios.
    MODIFIED: Forces Predicted ECG to use the threshold computed from Ground Truth.
    """
    ...
    # 4. Compute prediction ratio relative to Ground Truth (Prediction Ratio)
    if num_true > 0:
...
def analyze_p_wave_for_af(signal, fs=125, gt_threshold=None):
    # Pass Ground Truth threshold to peak detection
