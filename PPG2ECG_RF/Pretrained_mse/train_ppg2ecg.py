...
class TrainConfig:
    ...
    ecg_checkpoint_path: str = "/home/linhhima/PPG_ECG/PPG2ECG_RF/Proposed/saved_models_ecg_vae_subject/best_ecg_autoencoder.pth" 
    # --- PRE-TRAINED PPG CHECKPOINT PATH ---
    ppg_pretrained_path: str = "/home/linhhima/PPG_ECG/PPG2ECG_RF/Pretrained_mse/saved_models_ppg_vae_subject/best_ppg_autoencoder.pth"

    save_dir: str = "/home/linhhima/PPG_ECG/PPG2ECG_RF/Pretrained_mse/saved_models_alignment_subject"
    ...
    kl_weight: float = 1.0
    mse_weight: float = 1.0  # --- MSE LOSS WEIGHT ---
...
def plot_alignment_losses(history, save_dir, filename):
    ...
    # Arrange into 4 subplots in a single row
...
def build_frozen_ecg_teacher():
    ...
    # Handle safe globals for newer PyTorch versions
...
def load_pretrained_ppg_weights(model: nn.Module):
    """
    Loads pre-trained ppg2ppg weights into the Student Model for finetuning.
    """
    print(f"[*] Loading pre-trained PPG weights from: {CFG.ppg_pretrained_path}")
    if not os.path.exists(CFG.ppg_pretrained_path):
        print(f"[!] Warning: Pre-trained PPG checkpoint not found at {CFG.ppg_pretrained_path}. Model will be randomly initialized!")
        return model

    # Add custom class to safe globals if checkpoint contains custom objects
    ...
    # Map sub-modules from ppg2ppg.PPGAutoencoder into PPG2ECGModel
    model_dict = model.state_dict()
    mapped_state_dict = {}

    for k, v in state_dict.items():
        # Map from original encoder to ppg_encoder in alignment model
        if k.startswith("encoder."):
            new_k = k.replace("encoder.", "ppg_encoder.")
            if new_k in model_dict:
                mapped_state_dict[new_k] = v
        # Map from original latent_head to ppg_latent_head (Temporal Bottleneck)
...
def train_one_epoch(model, criterion, mse_criterion, optimizer, scaler, dataloader, epoch):
    ...
    for ecg, ppg, _ in loop:
        ...
        with autocast(enabled=CFG.use_amp):
            ...
            # 1. Calculate original Alignment Loss (CORAL + KL)
            align_losses = criterion(outputs)
            
            # 2. Calculate MSE Loss between PPG and ECG latent spaces
            # Assumes outputs returns "z_ppg" and "z_ecg" from PPG2ECGModel forward pass
            z_ppg = outputs["z_ppg"]
            z_ecg = outputs["z_ecg"]
            loss_mse = mse_criterion(z_ppg, z_ecg)

            # 3. Combine total loss with weights
...
def main():
    ...
    # 3. ENABLE FINETUNING: Load pre-trained PPG weights instead of training from scratch
    ...
    mse_criterion = nn.MSELoss().to(DEVICE)  # Initialize MSE Loss

    # 5. Optimizer & Scheduler (Only update parameters requiring gradients)
    optimizer = optim.AdamW(
        filter(lambda p: p.requires_grad, model.parameters()), 
        lr=CFG.lr, weight_decay=CFG.weight_decay
    )
    
    # Note: Removed deprecated 'verbose' parameter for compatibility with PyTorch 2.0+
