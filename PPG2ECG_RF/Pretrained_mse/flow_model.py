...
class SinusoidalPosEmb(nn.Module):
    """ Embeds time steps t into feature vectors using sinusoidal positional encoding """
...
class FiLMBlock1D(nn.Module):
    """ 1D ResNet block incorporating time embedding t via FiLM (Feature-wise Linear Modulation) mechanism """
    ...
    def forward(self, x, t_emb):
        # Compute scale and shift factors from time embedding
        scale, shift = self.mlp(t_emb).unsqueeze(-1).chunk(2, dim=1)
        
        res = x
        x = self.norm1(x)
        x = x * (scale + 1) + shift  # Inject time condition t into feature representation
...
class LatentRectifiedFlow(nn.Module):
    ...
    def __init__(self, latent_channels=16, cond_channels=16, hidden_dim=128, num_blocks=6):
        ...
        # Concatenate x_t and z_ppg (condition c) at the input layer
    ...
    def forward(self, xt, t, z_ppg):
        """
        xt: [Batch, 16, 75] - Noisy latent state interpolated with Target
        t: [Batch] - Time step continuously sampled from 0 to 1
        z_ppg: [Batch, 16, 75] - Aligned PPG latent representation (Conditioning)
        """
        t_emb = self.time_mlp(t)
        
        # Concatenate x_t and z_ppg along channel dimension
        ...
        out_v = self.conv_out(h) # Predict velocity vector field v
        return out_v