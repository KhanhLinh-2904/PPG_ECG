...
from PPG2ECG_RF.Proposed.load_data import LoadData
import random
import os
from PPG2ECG_RF.Proposed.metric import calculate_cosine_similarity, calculate_dtw_distance, calculate_metrics
from scipy.signal import correlate

# Import Model Configurations and Classes
from ecg2ecg import ECGAutoencoder, ECGAEConfig
from ppg2ecg import PPG2ECGModel, PPG2ECGConfig  # Updated import file name
...
def load_model():
    ...
    # 1. Configure & Load ECG Teacher (Required for PPG to use Decoder for reconstruction)
    ecg_cfg = ECGAEConfig(
    ...
    if not os.path.exists(PHASE1_MODEL_PATH):
        raise FileNotFoundError(f"Error: Phase 1 weights required to initialize Teacher: {PHASE1_MODEL_PATH}")
    ckpt1 = torch.load(PHASE1_MODEL_PATH, map_location=DEVICE)
    ecg_ae.load_state_dict(ckpt1.get("model_state_dict", ckpt1))
    
    # 2. Initialize PPG Student Configuration
    ...
    if not os.path.exists(PHASE2_MODEL_PATH):
        raise FileNotFoundError(f"Error: Phase 2 weights not found: {PHASE2_MODEL_PATH}")
...
def get_prediction(model, ppg_input):
    """ 
    Inference helper function: Extract z_ppg -> Pass through Teacher's Decoder -> Reconstructed ECG waveform.
    """
...
def align_signals(true_s: np.ndarray, pred_s: np.ndarray) -> np.ndarray:
    """ Computes cross-correlation to estimate phase lag and shift pred_s to align with true_s. """
