...
import matplotlib.pyplot as plt  # Added for plotting loss curves

# Import Stage 1 utility functions and model architectures
...
# ============================================================
# 1. STAGE 2 TRAINING CONFIGURATION
# ============================================================
@dataclass
class Stage2TrainConfig:
    ...
    ppg_checkpoint_path: str = "/home/linhhima/PPG_ECG/PPG2ECG_RF/Proposed/saved_models_alignment/best_ppg_alignment.pth"
    
    # Save directory and output filenames for Stage 2
    save_dir: str = "saved_models_flow"
    best_model_name: str = "best_rectified_flow.pth"
    plot_name: str = "flow_loss_curve.png"  # Filename for loss plot
...
def plot_flow_losses(train_losses, val_losses, save_dir, filename):
    ...
    plt.close()
    print(f"\n[+] Training loss plot saved at: {plot_path}")
...
def train_one_epoch(flow_model, ecg_ae, ppg_model, optimizer, scaler, dataloader, epoch):
    ...
    for ecg, ppg, _ in loop:
        ...
        # 1. EXTRACT LATENT REPRESENTATIONS FROM FROZEN MODELS
        with torch.no_grad():
        ...
        # Target vector field (Straight-line trajectory)
        v_target = z_ecg - z0
