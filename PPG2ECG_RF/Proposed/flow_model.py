...
class LatentRectifiedFlow(nn.Module):
    def __init__(self, latent_channels=16, cond_channels=16, hidden_dim=128, num_blocks=6):
        ...
        # concatenate x_t and z_ppg (Conditioning) 
    ...
    def forward(self, xt, t, z_ppg):
        ...
        xt: [Batch, 16, 75] 
        t: [Batch] 
        z_ppg: [Batch, 16, 75] (Conditioning)
        """
        t_emb = self.time_mlp(t)
        
        # concatenate x_t and z_ppg across channels
        ...
        out_v = self.conv_out(h) # predict vector velocity v
        return out_v
