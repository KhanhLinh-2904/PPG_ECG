...
def calculate_peak_count_ratio(true_s: np.ndarray, pred_s: np.ndarray, sampling_rate=125):
    ...
    """
    # 1. Detect R-peaks on Ground Truth ECG and EXTRACT THRESHOLD (gt_threshold)
...
def align_signals(true_s: np.ndarray, pred_s: np.ndarray) -> np.ndarray:
    """ 
    Finds phase lag and shifts pred_s to align with true_s via Cross-Correlation.
    """
...
# ==========================================
# MAIN EXECUTIONS (VISUALIZE, LOSS, SAVE, EVAL)
# ==========================================
def run_visualization(max_plots=100):
    """
    Plots comparison between reconstructed ECG and Ground Truth ONLY for segments
    belonging to record 'mimic_perform_non_af_001', computing and displaying P/R ratio.
    
    Args:
        max_plots (int, optional): Maximum number of segments to plot to prevent memory overload.
                                   Pass None to render all without limitation.
    """
    set_seed(42)
    ...
    try:
        ...
        # Keep shuffle=False to inspect sequentially from beginning to end
    ...
    with torch.no_grad():
        for batch_data in dataloader:
            # Process tensor dimensions and push to device (GPU/CPU)
            ...
            # Extract features and predict via Flow model
            ...
            # Iterate through individual segments in batch
            for i in range(ecg_true_np.shape[0]):
                rec_name = record_names[i]
                
                # Filter and render only if record name contains target string
                ...
                # =====================================================================
                try:
                    # Step 1: Extract standard threshold from Ground Truth ECG (fs=125)
                    ...
                    # Fallback if segment length is too short to locate peaks
                    p_ratio_str = "N/A"
                    p_peaks_idx, r_idx = [], []
                
                t = np.arange(len(ecg_gt_plot))
                time_ax = t / 125  # Convert to time axis (seconds)
                ...
                # 3. Predicted ECG (Reconstructed) + Detected P & R Peaks
                plt.subplot(4, 1, 3)
                plt.plot(t, ecg_pred_plot, color='red', label='Predicted ECG')
                
                # Highlight detected R-peaks and P-peaks on Reconstructed ECG plot
                if len(r_idx) > 0:
                ...
                # Check stopping condition if max_plots limit is reached
                if max_plots is not None and plot_count >= max_plots:
                    print(f"\n[*] Reached configured limit of {max_plots} segments for {target_record}. Stopping execution!")
                    return

    print(f"\n[*] Processing complete. Found and displayed a total of {plot_count} segments for {target_record}.")
...
def run_loss():
    ...
    # Helper to create empty metric tracker dictionary
    ...
    # Dictionary storing results per dataset and overall total
    ...
    with torch.no_grad():
        for batch_data in tqdm(test_loader, desc="Evaluating Metrics"):
            ...
            # Retrieve record names for dataset categorization
            record_names = batch_data[2] if len(batch_data) > 2 else [f"unknown_{i}" for i in range(ecg.shape[0])]
            ...
            for b in range(ecg_true_np.shape[0]):
                ...
                # 1. Calculate metrics before phase alignment
                ...
                frechet_b = calculate_frechet_distance(true_s, pred_s)
                
                # Perform phase alignment
                aligned_pred_s = align_signals(true_s, pred_s)
                
                # Calculate metrics after phase alignment
                ...
                frechet_a = calculate_frechet_distance(true_s, aligned_pred_s)
                ...
                # 3. Helper to accumulate metrics in dictionary
                ...
                # Always accumulate overall metrics
...
def run_peak_count_evaluation():
    set_seed(SEED)
    # Load Stage 1 & 2 models for Flow generation
    ecg_ae, ppg_model, flow_model = load_models()
    ...
    # Initialize tracking dictionaries for target datasets
    target_datasets = ["dalia", "wesad", "bidmc", "capno", "mimic"]
    ...
    print("[*] Counting and computing R-peak error metrics (MAE & Error Percentage)...")
    ...
    with torch.no_grad():
        for batch_data in tqdm(test_loader, desc="Evaluating R-Peaks Errors"):
            ...
            for b in range(ecg_true_np.shape[0]):
                ...
                # Helper to update tracker counters
...
if __name__ == "__main__":
    # SELECT 1 OF THE FUNCTIONS BELOW TO EXECUTE (Uncomment to use):
