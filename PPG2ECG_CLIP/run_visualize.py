...
CLIP_MODEL_PATH = "/home/linhhima/PPG_ECG/PPG2ECG_CLIP/best_multitask_model_subject.pth" 
...
def run_tsne_layer_evolution():
    ...
    # ---------------------------------------------------------
    # 3D t-SNE (SCATTER PLOT) for FINAL LATENT
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 3D GAUSSIAN HISTOGRAM (DENSITY SURFACE)
    # ---------------------------------------------------------
    ...
    #  2D (100x100) matrix to calculate density
    X, Y = np.mgrid[x_min:x_max:100j, y_min:y_max:100j]
    positions = np.vstack([X.ravel(), Y.ravel()])

    # KDE (Kernel Density Estimation) for PPG
    ...
    # Surface Plot
    # cmap Blues for ECG and Reds for PPG
